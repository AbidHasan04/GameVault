...
if (isset($_SESSION['name'])) {
    $errorLog = 'You are already Logged in';} else{
if(isset($_POST['submit'])){
    ...
    if(mysqli_num_rows($res) >0){
        ...
        $sel = "SELECT Name, UserID FROM users WHERE Name = '$name';";
...
<div class="MakeAccount">
    ...
    <?php
    if (isset($errorLog)) {
        echo ' <h2>Whoops!</h2><p>' .$errorLog.'  ' .$_SESSION['name'].'.</p>';} else {
        if (isset($error)) {
             
                echo '<span style = "margin-left:10px;">' . $error . '</span>';
            
        } else {
            echo '<p class="Text">You can only download if youre logged in.</p>';
        }
